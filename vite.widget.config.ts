...
// Public Turnstile site key — baked into the bundle. Empty string
// means Turnstile is OFF for this build (graceful mode: widget
// submits without a token, server bypasses the gate when
// TURNSTILE_SECRET is also unset).
const TURNSTILE_SITEKEY = process.env.VITE_FB_TURNSTILE_SITEKEY ?? ''
